...
class _TankbeurtScreenState extends State<TankbeurtScreen> {
  ...
  final List<String> _quotes = [
    ...
    "Soepel schakelen is jouw thing.", "Jij bent de turbo in mijn day.", "Niet morsen he...", "Klaar voor de start?", "Jouw glimlach is mijn brandstof.",
...
class _ConsumptionDetailPageState extends State<ConsumptionDetailPage> {
  ...
  Widget build(BuildContext context) {
    final data = context.watch<DataProvider>();
    final chartData = data.getConsumptionHistory(widget.carId);
    double averageVal = data.getStats(widget.carId)['avgCons'] ?? 0.0;

    double screenWidth = MediaQuery.of(context).size.width - 48; 
    int gridCount = max(chartData.length, 10);
    double slotWidth = screenWidth / 10;
    double totalChartWidth = slotWidth * gridCount;
    double barWidth = slotWidth * 0.8;
      ...
      body: ListView(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text("Trend", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ...
          SingleChildScrollView(
            ...
            child: Container(
              ...
              child: chartData.isEmpty 
                ? const Center(child: Text("Nog niet genoeg data (min. 2 tankbeurten)"))
                : isBarChart 
                  ? _buildBarChart(chartData, averageVal, gridCount.toDouble(), barWidth) 
                  : _buildLineChart(chartData, averageVal, gridCount.toDouble()),
            ),
          ),
          
          const SizedBox(height: 32),
          const Text("Historie", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          ...chartData.reversed.map((item) => Card(
            margin: const EdgeInsets.only(bottom: 12),
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            child: ListTile(
              leading: CircleAvatar(
                backgroundColor: Colors.blueAccent.withValues(alpha: 0.1), 
                child: const Icon(Icons.local_gas_station, size: 20, color: Colors.blueAccent)
              ),
              title: Text("1 op ${(item['val'] as double).toStringAsFixed(2)}"),
              subtitle: Text("${item['distance'].toStringAsFixed(0)} km met ${item['liters']} L"),
  ...
  double _calculateMaxY(double avg, List<Map<String, dynamic>> data) {
    double maxVal = 0;
    if (data.isNotEmpty) {
      maxVal = data.map((e) => e['val'] as double).reduce(max);
    }
    double base = maxVal > avg ? maxVal : avg; 
    return ((base + 5) / 5).ceil() * 5.0;
  }

Widget _buildBarChart(List<Map<String, dynamic>> data, double avg, double maxX, double barW) {
    double maxY = _calculateMaxY(avg, data);
    return BarChart(BarChartData(
        ...
        extraLinesData: ExtraLinesData(horizontalLines: [
          HorizontalLine(y: avg, color: Colors.green, strokeWidth: 2, dashArray: [4, 4], label: HorizontalLineLabel(show: true, labelResolver: (l) => "Gem", style: const TextStyle(color: Colors.green, fontSize: 10, fontWeight: FontWeight.bold), alignment: Alignment.topRight))
        ]),
        titlesData: FlTitlesData(
          leftTitles: const AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 30, interval: 5)),
          bottomTitles: AxisTitles(sideTitles: SideTitles(
            ...
            getTitlesWidget: (val, meta) {
              ...
              if (idx >= 0 && idx < data.length) {
                return Padding(padding: const EdgeInsets.only(top: 4), child: Text(_formatDate(data[idx]['date']), style: const TextStyle(fontSize: 10)));
              }
        ...
        barTouchData: BarTouchData(
          ...
          touchTooltipData: BarTouchTooltipData(
            // DIT IS DE FIX: getTooltipColor in plaats van tooltipBgColor
            ...
            getTooltipItem: (group, groupIndex, rod, rodIndex) {
              return BarTooltipItem(
                ...
                const TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 11),
              );
            },
          ),
        ),
        borderData: FlBorderData(show: true, border: Border(bottom: BorderSide(color: Colors.grey.withValues(alpha: 0.3)), left: BorderSide(color: Colors.grey.withValues(alpha: 0.3)))),
      ));
  }

  Widget _buildLineChart(List<Map<String, dynamic>> data, double avg, double maxX) {
    double maxY = _calculateMaxY(avg, data);
    return LineChart(LineChartData(
        ...
        extraLinesData: ExtraLinesData(horizontalLines: [
          HorizontalLine(y: avg, color: Colors.green, strokeWidth: 2, dashArray: [4, 4])
        ]),
        titlesData: FlTitlesData(
          leftTitles: const AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 30, interval: 5)), 
          bottomTitles: AxisTitles(sideTitles: SideTitles(
        ...
        lineTouchData: LineTouchData(
          ...
          touchTooltipData: LineTouchTooltipData(
            // DIT IS DE FIX: getTooltipColor callback
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: LayoutBuilder(builder: (context, constraints) {
        return SingleChildScrollView(child: ConstrainedBox(constraints: BoxConstraints(minHeight: constraints.maxHeight), child: Column(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Padding(padding: const EdgeInsets.all(20), child: GestureDetector(onTap: () { if (++_clicks >= 7) { _clicks = 0; bool isDeLorean = data.cars.any((c) => c.licensePlate?.toUpperCase() == 'OUTATIME'); widget.onEasterEgg(isDeLorean); } }, child: const Text("TankBuddy v1.0.74", style: TextStyle(color: Colors.grey)))),
              ],
            )));
      },
    ));
